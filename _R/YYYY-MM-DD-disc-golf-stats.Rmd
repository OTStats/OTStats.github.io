...
Players such as Paul Mcbeth, Ricky Wysocki, Eagle McMahon, and Calvin Heimburg were regularly appearing in the final rounds of events of the men's events, and Paige Pierce continually dominated the women's division.
...
Luckily the PDGA, the main governing body of professional disc golf, tracks most of this information. 
The PDGA website [Player Statistics](https://www.pdga.com/players/stats) page tracks annual earnings, ratings, and points for players for all PDGA-sanctioned events back to 1979. 
There didn't appear to be any convenient way for me to compare these player's earnings over time so I saw this as an opportunity to practice webscraping.
...
```
Let's start by just scraping a single page to understand the structure of the webpage (i.e. find the table within the HTML code). 
It took me about an hour to remind myself of how to use inspect to find the breadcrumb path to the html table, and I decided against including documentation about the process here. a) because I didn't want to, b) I don't remember, c) there are plenty of very useful tutorials already online.
```{r load libraries}
library(tidyverse)
library(rvest)
library(xml2)
